...
class TasksPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          Container(
            ...
            child: Row(children: [_ViewToggleButton(icon: Icons.list, isActive: !controller.isKanbanMode, onTap: () => !controller.isKanbanMode ? null : controller.toggleViewMode()), Container(width: 1, height: 20, color: borderColor), _ViewToggleButton(icon: Icons.view_kanban, isActive: controller.isKanbanMode, onTap: () => controller.isKanbanMode ? null : controller.toggleViewMode())]),
          ),
      ...
      floatingActionButton: FloatingActionButton.extended(
        backgroundColor: primaryColor, elevation: 2, icon: const Icon(Icons.add, color: Colors.white, size: 20),
  ...
  // ... (Métodos de UI: _buildListBody, _buildKanbanBody, etc. - Mantidos iguais, apenas lógica interna muda)
  // Vou abreviar as partes repetitivas de layout para focar na lógica que quebrava, 
  // mas o código completo da classe segue abaixo:

  Widget _buildListBody(BuildContext context, TaskController controller, bool canAddCategory) {
    return SingleChildScrollView(
      padding: const EdgeInsets.symmetric(horizontal: 32, vertical: 24),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text("Prod Works", style: GoogleFonts.inter(fontSize: 24, fontWeight: FontWeight.w700, letterSpacing: -1)), Text("VISÃO LISTA", style: GoogleFonts.jetBrainsMono(color: Colors.grey, fontSize: 12, fontWeight: FontWeight.bold))]),
          ...
          if (controller.completedTasks.isNotEmpty) ...[_buildSectionHeader(context, "CONCLUÍDAS", controller.completedTasks.length, isGrey: true), const SizedBox(height: 16), Opacity(opacity: 0.6, child: _buildGrid(controller.completedTasks, controller))]
      ]),
    );
  }
  ...
  Widget _buildKanbanBody(BuildContext context, TaskController controller, bool canAddCategory) {
    ...
    return Column(children: [
        Padding(padding: const EdgeInsets.fromLTRB(32, 24, 32, 16), child: Column(children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text("Fluxo de Trabalho", style: GoogleFonts.inter(fontSize: 24, fontWeight: FontWeight.w700, letterSpacing: -1)), Text("VISÃO KANBAN", style: GoogleFonts.jetBrainsMono(color: Theme.of(context).primaryColor, fontSize: 12, fontWeight: FontWeight.bold))]), const SizedBox(height: 16), _buildFilters(context, controller, canAddCategory)])),
        Expanded(child: ListView(scrollDirection: Axis.horizontal, padding: const EdgeInsets.symmetric(horizontal: 32), children: [
              _buildKanbanColumn(context, "TO DO", TaskStatus.todo, allFilteredTasks, controller), const SizedBox(width: 16),
              _buildKanbanColumn(context, "IN PROGRESS", TaskStatus.inProgress, allFilteredTasks, controller), const SizedBox(width: 16),
              _buildKanbanColumn(context, "REVIEW", TaskStatus.review, allFilteredTasks, controller), const SizedBox(width: 16),
  ...
  Widget _buildKanbanColumn(BuildContext context, String title, TaskStatus status, List<TaskModel> allTasks, TaskController controller) {
    ...
    return DragTarget<String>(
      ...
      builder: (context, _, __) => Container(width: 320, decoration: BoxDecoration(color: Theme.of(context).cardTheme.color!.withOpacity(0.5), borderRadius: BorderRadius.circular(12), border: Border.all(color: Theme.of(context).dividerTheme.color!)), child: Column(children: [
              Padding(padding: const EdgeInsets.all(16), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Row(children: [Container(width: 8, height: 8, decoration: BoxDecoration(color: statusColor, shape: BoxShape.circle)), const SizedBox(width: 8), Text(title, style: GoogleFonts.jetBrainsMono(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.grey[400]))]), Text(tasks.length.toString(), style: GoogleFonts.jetBrainsMono(fontSize: 10, color: Colors.grey))])),
              const Divider(height: 1),
              Expanded(child: ListView.separated(padding: const EdgeInsets.all(12), itemCount: tasks.length, separatorBuilder: (_, __) => const SizedBox(height: 12), itemBuilder: (ctx, i) { final task = tasks[i]; return LongPressDraggable<String>(data: task.id, feedback: SizedBox(width: 280, child: Opacity(opacity: 0.9, child: TaskCard(task: task, onToggle: () {}))), childWhenDragging: Opacity(opacity: 0.3, child: TaskCard(task: task, onToggle: () {})), child: TaskCard(task: task, onToggle: () => controller.toggleTaskCompletion(task.id))); })),
      ])),
    );
  }

  Widget _buildFilters(BuildContext context, TaskController controller, bool canAddCategory) {
    return SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: [...controller.categories.map((c) => _buildFilterTab(context, c.label, c.icon)), if (canAddCategory) Padding(padding: const EdgeInsets.only(left: 8), child: Tooltip(message: "Adicionar Nova Categoria", child: InkWell(onTap: () => _showAddCategoryDialog(context), borderRadius: BorderRadius.circular(6), child: Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8), decoration: BoxDecoration(color: const Color(0xFF1F2937), borderRadius: BorderRadius.circular(6), border: Border.all(color: const Color(0xFF374151))), child: Row(children: [Icon(Icons.add, size: 16, color: Colors.grey.shade400), const SizedBox(width: 4), Text("Nova", style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.grey.shade400))])))))]));
  }
  ...
  // [CORREÇÃO FINAL AQUI]: Recebe String e Converte Subtasks
  Future<void> _showAddTaskDialog(BuildContext context) async {
    final result = await showDialog(context: context, builder: (_) => const AddTaskDialog());
    
    if (result != null && context.mounted) {
      // 1. Converter lista de Strings em lista de SubTask
      ...
      // 2. Criar tarefa (category já é String)
